...
export const writeNFCUser = async (request: Pick<User, 'username' | 'role_id' | 'institution_id'>, nfcDevice: NFCDevice) => {
  try {
    console.info('[REQ WRITE NFC USER]', request);
    const response = await sendRequestPOST(
      `http://${nfcDevice.ipAddress}:${nfcDevice.port}${endpoint.nfc.writeUser}`, request
    );
    return response;
  } catch (error: any) {
    console.error('[WRITE NFC USER ERROR]', error);
    showNotification({
      color: 'red',
      title: 'Request Error',
      message: error?.response?.data?.message || error?.response?.data || error?.message,
