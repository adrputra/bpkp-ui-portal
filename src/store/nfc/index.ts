...
import { pingNFCDevice, scanNFCTag, writeNFCTag, writeNFCUser } from '@/api/nfc';
...
interface NFCStore {
  ...
  writeNFCUser: (userDetail: User) => Promise<void>;
}
...
export const useNFCStore = create<NFCStore>()((set) => ({
  ...
  writeNFCUser: async(userDetail: User) => {
    await writeNFCUser(userDetail, useNFCStore.getState().nfcDevice).then((res) => {
      if (res.code === 200) {
        showNotification({
          title: 'Successfully Write NFC Tag',
          message: 'You can now Read/Write NFC Tag',
